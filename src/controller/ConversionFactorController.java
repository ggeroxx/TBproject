...
public class ConversionFactorController extends Controller {
    ...
    public void print ( ConversionFactor cf ) throws SQLException
    {
        ...
        String rootLeaf1 = "";
        if ( categoryController.getCategoryJDBC().getNumberOfEqualsCategories( cf.getLeaf_1() ) > 1 )
            rootLeaf1 = "  [ " + categoryController.getCategoryJDBC().getRootByLeaf( cf.getLeaf_1() ).getName() + " ]  ";

        String rootLeaf2 = "";
        if ( categoryController.getCategoryJDBC().getNumberOfEqualsCategories( cf.getLeaf_2() ) > 1 )
            rootLeaf2 = "  [ " + categoryController.getCategoryJDBC().getRootByLeaf( cf.getLeaf_2() ).getName() + " ]  ";
